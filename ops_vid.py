...
class PasteVertID():
    # invert_normals: bpy.props.BoolProperty(name="Invert Normals",
    # description="Invert Normals", default=False)

    @staticmethod
    def sortOtherVerts(processedVertsIdDict, preocessedEdgesIsDict,
                       preocessedFaceIsDict, bm):
        """Prevet verts on other islands from being all shuffled"""
        # dicts instead of lists - faster search 4x?
        if len(bm.verts) == len(processedVertsIdDict) and len(bm.faces) == len(
                preocessedFaceIsDict):
            return
            # all verts, and faces were processed - > no other Islands -> quit
        ...
        def fix_islands(processed_items, bm_element):  # face, verts, or edges
            ...
            notProcessedItemsIds = {
                ele.index: 1 for ele in bm_element if ele not in
                processedItems}
            ...
            for item in notProcessedElements:
                if item.index in processedIDs:
                    # if duplicated id found in not processed verts
    ...
    def execute(self):
        ...
        for i, _ in enumerate(all_sel_faces):
            ...
            if all_sorted_faces:
                ...
                for j, (face, face_data) in enumerate(
                        all_sorted_faces.items()):
