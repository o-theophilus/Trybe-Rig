...
class VOT_OT_TransferVertId(bpy.types.Operator):
    ...
    delta: bpy.props.FloatProperty(
        name="Delta", description="SearchDistance", default=0.1, min=0, max=1, precision=4)
    ...
    def execute(self, context):
        ...
        if not TargetObjs:
            self.report(
                {'ERROR'}, 'You seed to select two mesh objects (source then target that will receive vert order)! Cancelling')
            return {'CANCELLED'}
